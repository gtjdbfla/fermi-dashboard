...
heading(
    "핵심 판정", size="####",
    help_text=(
        "전력·에너지 인프라 **13개사를 놓고 살아남은 곳과 무너진 곳을 탐색적으로 비교**한 "
        "결과다. 표본이 13곳으로 작고, 커버리지의 분모(반입·목표 등)가 회사마다 다르며, "
        "흑자 전환 판정도 이미 벌어진 일을 사후에 본 것이라 **예측력이 검증됐다고 보기는 "
        "어렵다.** 판별력이 낮았던 항목(런웨이·이자 자본화·희석)은 **참고 지표** 탭에 "
        "남겨 뒀다 — 주주가 감수하는 위험을 보는 데는 여전히 핵심적이다.\n\n"
...
with tabs[8]:
    ...
    heading("개별 증권사 액션", help_text=(
        ...
        "**목표가 뒤의 `(MM-DD 기준)`은 이어붙인 값이다.** 소스가 '유지' 등급에는 목표가를 싣지 "
        "않는데, 유지는 목표가를 바꾸지 않았다는 뜻이므로 그 증권사의 직전 목표가가 그대로 "
        "유효하다. 비워 두면 '최근 리포트에 목표가가 없다'로 읽혀 오해를 부른다. 이렇게 이어붙인 "
        "값들의 평균이 컨센서스(평균 $10.60 · 저 $6 · 고 $17)와 정확히 맞는다."))
    if actions.empty:
