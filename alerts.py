...
WATCHED_ITEMS = {
    ...
    "8.01": "기타 중요사건",
    # 아래 셋은 페르미에 아직 없다. 다만 나오면 매우 무겁고, 정기보고서 본문보다
    # **8-K가 먼저 온다** — 회계법인 교체와 재작성은 분기보고서를 기다리지 않는다.
    "3.01": "상장규정 위반·상장폐지 통지",
    "4.01": "회계법인 변경",
    "4.02": "기존 재무제표 신뢰 불가(재작성)",
    # 파산과 기한이익상실은 이 회사에서 가장 무거운 코드다. 2.04는 **약정 위반이
    # 실제로 발동했을 때 오는 자리**다 — 400MW 미서명으로 상환 조건이 바뀌면
    # 여기로 나올 수 있다. 2.06은 손상차손 인식이고, 정기보고서보다 먼저 온다.
    "1.03": "파산·법정관리",
...
# 그것만으로 이름을 붙이면 나머지는 AI에게 그냥 숫자로 간다. 실제로 2026-08-31
# 8-K가 `Item 5.08,8.01 = 기타 중요사건`으로 전달돼, AI가 "8-K가 접수되었으나"라고만
# 쓰고 첫 정기주총 결정이라는 알맹이를 놓쳤다. 알리지 않는 코드도 이름은 알아야 한다.
ITEM_MEANING = {
    "1.01": "중요 계약 체결", "1.02": "중요 계약 해지", "1.03": "파산·법정관리",
    "1.04": "광산 안전", "1.05": "중대 사이버 침해",
    "2.01": "인수·처분 완료", "2.02": "실적 발표", "2.03": "채무·의무 발생",
    "2.04": "기한이익상실·조기상환 의무 발생", "2.05": "사업 중단·구조조정 비용",
    "2.06": "손상차손 인식",
    "3.01": "상장규정 위반·상장폐지 통지", "3.02": "미등록 지분 매각(희석)",
    "3.03": "주주 권리 변경",
    "4.01": "회계법인 변경", "4.02": "기존 재무제표 신뢰 불가(재작성)",
    "5.01": "지배권 변경", "5.02": "임원·이사 변동", "5.03": "정관·회계연도 변경",
    "5.04": "연금 거래정지", "5.05": "윤리강령 변경", "5.06": "셸 지위 변경",
    "5.07": "주주 표결 결과", "5.08": "주주 이사후보 지명·주총 일정",
    "7.01": "Reg FD 공시", "8.01": "기타 중요사건", "9.01": "재무제표·첨부자료",
}


def item_names(items: str) -> str:
    """'5.08,8.01' → '주주 이사후보 지명·주총 일정, 기타 중요사건'."""
    out = [ITEM_MEANING.get(code.strip(), code.strip())
           for code in str(items or "").split(",") if code.strip()]
    return ", ".join(out)
